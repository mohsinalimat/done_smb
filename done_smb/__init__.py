...
def leave_allocate_annual(from_d,to_d, mm):
		import datetime 
		import calendar
		import math
		from datetime import date, timedelta
		today = date.today()
		first_date = today.replace(day=from_d)
		if first_date == today:
			...
			for employee in employees:
				...
				leave_alloc_doc.to_date = today.replace(day = to_d,month=mm)#datetime.date(int(today_list[2]), int(today_list[1]), int(last_date))
